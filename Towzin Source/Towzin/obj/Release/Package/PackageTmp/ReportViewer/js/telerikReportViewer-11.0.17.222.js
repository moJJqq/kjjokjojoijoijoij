...
(function(trv, $, window, document, undefined) {
    ...
    function ReportViewerController(options) {
        var controller = {}, $controller = $(controller), clientId, reportInstanceId, reportDocumentId, registerClientDfd, reportInstanceDfd, getDocumentFormatsDfd, report, parameterValues, currentPageNumber, pageCount, viewMode = trv.ViewModes.INTERACTIVE, documentFormats, loader, printMode = trv.PrintModes.AUTO_SELECT, bookmarkNodes, clientHasExpired = false, parameterValidators = trv.parameterValidators;
        clearReportState();
        ...
        $.extend(controller, {
            ...
            refreshReport: function(ignoreCache, baseDocumentId, actionId) {
                ...
                var promise = controller.loadParameters(null);
                $.when(promise).done(function(p) {
                    var params = {};
                    var hasError = false;
                    $.each(p || [], function() {
                        try {
                            params[this.id] = parameterValidators.validate(this, this.value);
                        } catch (e) {
                            hasError = true;
                            return;
                        }
                ...
                controller.reloadParameters(promise);
            },
...
(function(trv, $, window, document, undefined) {
    ...
    function ParametersArea(placeholder, options, otherOptions) {
        ...
        function updateParameters(successAction) {
            loadParameters(controller.loadParameters(recentParameterValues), successAction);
        }
        function loadParameters(controllerLoadParametersPromise, successAction) {
            beginLoad();
            $.when(controllerLoadParametersPromise).done(function(parameters) {
        ...
        function onReloadParameters(event, controllerLoadParametersPromise) {
            showError();
            $content.empty();
            loadParameters(controllerLoadParametersPromise, onLoadParametersSuccess);
        }
